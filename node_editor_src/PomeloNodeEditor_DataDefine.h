...
namespace pne_datadefine {
	...
	struct node_in_connectpoint {
		...
		int32_t link_number; // line id, unique
		
		// 深拷贝 node_in_connectpoint.
		node_in_connectpoint& operator=(const node_in_connectpoint& other) {
			if (this != &other) {

				point_number = other.point_number;
				point_color  = other.point_color;
				point_text   = other.point_text;

				point_param_type = other.point_param_type;
				point_param      = other.point_param;

				link_number = other.link_number;
			}
			return *this;
		}
	};
	...
	struct node_out_connectpoint {
		...
		std::vector<int32_t> link_number; // line id, multiple
		
		// 深拷贝 node_out_connectpoint.
		node_out_connectpoint& operator=(const node_out_connectpoint& other) {
			if (this != &other) {
	...
	struct node_attribute {
		...
		// 深拷贝 node_attribute.
		node_attribute& operator=(const node_attribute& other) {
			if (this != &other) {

				node_number      = other.node_number;
				node_color       = other.node_color;
				node_color_click = other.node_color_click;
				node_title       = other.node_title;

				node_tex = other.node_tex;

				in_connect  = other.in_connect;
				out_connect = other.out_connect;

				node_numtype = other.node_numtype;
			}
			return *this;
		}
	};
